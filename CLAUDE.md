...
An exploration is more credible when it reports something the author got wrong. Prefer a mistake from the same domain as the question. A mistake from another domain works only when it explains why the author now distrusts a specific assumption.

Place the mistake where it carries the argument. Not in the introduction, and not as an aside about another project. If the exploration argues for building less, the mistake belongs in the section that makes that argument. A mistake that could be deleted without weakening the reasoning is decoration.

Do not invent experience. If the record doesn't establish what the author thought or did, ask before writing the claim.

### Ending without a verdict

An exploration may reach a provisional answer and still be unfinished. Keep the answer, then state the decision that is still open.

Name the options and their costs, and don't declare a winner when the author hasn't chosen one. A reader who can see three named options can pick one and explain why. A reader who is handed a finished conclusion has nothing to answer.

Avoid restating the conclusion in slightly different words. A closing section that only repeats the thesis ends the piece as an essay rather than as an experiment.

### Giving the reader a way to answer

An exploration that expects replies needs a reply path on the page. Ask for something specific: a decision the reader has already made, and what it cost. Avoid a general invitation to share thoughts.

Name what would change the author's mind. A reader who can see the disconfirming case has a reason to write.
