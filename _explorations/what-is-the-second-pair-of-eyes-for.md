...
But not every company made approval easier. Shopify cleared about 70% of its security backlog in eleven days without removing the human approver. They simply automated the creation of the fixes, added an explaination of the fixand why it is needed, and keeps it ready to merge by a human. Monzo, a licensed bank, built an agent to do the writing and kept the human approver. What they found is that the shortage did not go away. It moved. The hard part used to be finding an engineer with capacity. Now it is finding a reviewer.
