...
Until June 2025 in Norway, a system could not go live before the responsible one had approved it. The rule never said who that was, and each firm decided for itself. It said "one". However, since July 2025, payment and e-money institutions follow DORA instead. It requires "the independence of the functions that approve changes and the functions responsible for requesting and implementing those changes.".

A function does not have to be a person. The rule never says who, or what, the approving function is. Basel leaves the same room when it defines dual control, calling it two or more separate "entities (usually persons)" acting in concert.

The rule doesn't ask the questions a developer would ask. It requires that every change is recorded, tested, assessed, approved, implemented and verified in a controlled manner. It requires that the function approving a change is independent from the function that asked for it. It never asks whether the approver understood the change. And where the same regulation does require "source code reviews", it defines them as static and dynamic testing, which is a scanner rather than a person reading the change.

How carefully a change is checked can also depend on its risk. The policy has to be "based on a risk assessment approach", and the regulation asks firms to account for their size, risk profile, and complexity, naming change management directly. A reversible configuration change and a change to how money moves don't have to receive the same scrutiny.

I went looking for the rule that says a human must approve, and I didn't find it. When the European supervisors wrote about the risks of frontier AI models in July 2026, the statement never used the words approval, human, segregation, or four eyes. What it asked for was more automation in development and deployment. Nobody has written that a machine may approve a change, and nobody has written that it may not.

So there are three answers to where the second pair of eyes came from. Fagan's inspections wanted defects found early. The maintainer wanted to decide what entered code he would live with. The regulator wants an independent function to approve, a record of what changed, and a depth of checking that follows risk. None of the three is the same requirement. Teams today satisfy all three with one pull request and one approval, and none of the three was written with a machine in mind.
