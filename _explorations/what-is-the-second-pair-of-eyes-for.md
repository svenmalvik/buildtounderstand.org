...
But not every company made approval easier. Shopify cleared about 70% of its security backlog in eleven days without removing the human approver. The speed came from a system that writes each fix, explains why it is needed, and keeps it ready to merge while it waits. The engineer who owns the code is left with the decision, not the work around it. Monzo, a licensed bank, kept the approver too and reported only that the hard part moved from finding an engineer to finding a reviewer.
