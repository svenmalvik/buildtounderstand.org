...
Most code reviews today happen inside a pull request. Someone opens a change, someone else reads it and approves it, and the branch is merged. The pull request has"" become the review tool, and the approval has become the second pair of eyes. But where did the pull request come from? And is it still worth having when the change was written by an AI?

In February 2002, the Linux kernel developer Jeff Garzik wrote a short guide for other developers who wanted their changes in the kernel. A change only became part of the kernel when Linus Torvalds pulled it into his repository. The guide said: publish your changes in a repository on the internet that Linus can pull from. Write a summary. Add a diffstat, which lists the files you changed and how much. These rules existed to save Linus work. Many developers sent him changes, and he had to read and sort all of them himself. A clear description made that sorting faster. Keeping unrelated changes apart meant he could pull one group while another was still under discussion.

The word pull describes what the maintainer does. The contributor pushes the change to a repository they own and then asks the maintainer to pull it. The maintainer can pull it, or leave it. The contributor had no permission to write to the maintainer's repository. So from the beginning, writing a change and accepting it were two acts, done by two different people. Nobody called this a control. but just how the work was split.

GitHub turned this into a feature in February 2008. A pull request on GitHub was a message that told someone there was work to pull. In August 2010, GitHub released "Pull Requests 2.0". It put the changes and the discussion about them on one page, and it allowed pull requests between branches of the same repository. GitHub called the redesign "our take on code review." The approval rules came after that: the merge button in 2011, protected branches in 2015, required reviews in 2016, and code owners in 2017. Each of these rules was added to a workflow that had worked without it for years.

Pull requests between branches of the same repository changed why people opened them. In the kernel, a developer opened a pull request because they couldn't write to Linus's repository. Inside a company, everyone on the team can write to the shared repository. When a team opens a pull request against its own master branch (now most call it the main branch), it doesn't need permission to integrate because it had already that permission.
